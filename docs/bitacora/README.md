...
### [2026-09-18] - Fix: columna `creado_en` inexistente en el dashboard de analíticas
- **Módulo(s)**: `analiticas`
- **Descripción**: Corregido el bug diagnosticado en la entrada del 2026-09-15. `CasoUsoAnaliticas` filtraba/agrupaba por la columna `creado_en`, que nunca existió en la tabla `ordenes` (la real es `created_at`, definida en `V5__create_orders.sql` y nunca renombrada en las 18 migraciones) — dos de las tres queries del dashboard habrían fallado en producción con `PSQLException: column "creado_en" does not exist`. El PR #22 (Fase 13) ya se mergeó a `master`; no había ninguna otra fase formal pendiente en el ROADMAP ni en `docs/superpowers/plans/`, así que este bug ya diagnosticado era el siguiente paso más claro.
- **Archivos modificados/creados**:
  - `src/main/java/com/ecommerce/modulos/analiticas/application/CasoUsoAnaliticas.java` (fix: `creado_en` → `created_at`)
  - `src/test/java/com/ecommerce/modulos/analiticas/application/CasoUsoAnaliticasTest.java` (constante `FRAGMENTO_SQL_VENTAS_DIARIAS` actualizada para seguir matcheando el SQL real)
  - `src/test/java/com/ecommerce/modulos/analiticas/application/CasoUsoAnaliticasIntegrationTest.java` (nuevo — ejecuta las 3 queries contra un Postgres real vía Testcontainers)
- **Decisiones técnicas**: El test unitario existente mockea `JdbcTemplate` por completo, por eso el bug pasó CI sin detectarse pese a "cobertura completa" del paquete. Se agregó un test de integración (mismo patrón que `CasoUsoOrdenIntegrationTest`: Testcontainers + `ddl-auto=update`, que genera el esquema desde las entidades JPA) que persiste una `Orden` real vía `RepositorioOrden.saveAndFlush(...)` y llama a `obtenerResumen(...)` de punta a punta — esto sí habría atrapado el bug, porque `EntidadBase` mapea `@Column(name = "created_at")` igual que Flyway.
- **Validación realizada**: `./gradlew test --tests "com.ecommerce.modulos.analiticas.*"` y `./gradlew clean test` completo. El nuevo `*IntegrationTest` depende de Testcontainers, que localmente falla por el problema conocido de Docker Desktop documentado en `CLAUDE.md` (no relacionado a este fix) — CI es la fuente de verdad, igual que el resto de los `*IntegrationTest` del repo.
- **Próximos pasos**: continúa pendiente, sin plan escrito todavía, agregar gitleaks/Trivy/SpotBugs a CI.
